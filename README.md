...
- configurable dashboard metrics and date periods for each project;
...
- reusable frontend VibeLMS header, footer, language switcher and Elementor widgets, including the language-aware **Контент по языку** widget for separate RU/KZ (or project-defined) blocks;
...
VIBELMS_PACKAGE_VERSION=0.0.55 ./scripts/build-installable-package.sh
```

The resulting `dist/vibelms-0.0.55.zip` can be uploaded in **Plugins → Add New → Upload Plugin** on a staging WordPress site. Advanced Quizzes is bundled into VibeLMS, so a separate paid add-on is not required. Its PHP and browser messages are included in the Russian catalog. After activation, create a test under **VibeLMS → Тесты**, attach it to a lesson, and insert it with the Elementor widget **Тест**. Courses, lessons and tests also support **Редактировать в Elementor**; public Elementor content is now rendered by the shared LifterLMS content pipeline with access checks and a fallback to the saved post content, including the direct full-description path used by course pages. For a course, the Elementor Finder contains **Открыть конструктор курса VibeLMS**, which opens the existing `llms-course-builder` in a separate tab for course structure editing. The widget **Результаты тестов** can render either the participant’s result list or the complete result page for a selected test, including the configurable certificate button. Every widget also has a **Стили VibeLMS** tab with typography, colors, background, responsive spacing, border and shadow controls; the shared typography settings apply to widget headings and to heading links as well as the widget wrapper. The header and footer additionally expose their content, links, logo and button settings. Create slides, videos and documents under **VibeLMS → Материалы**, choose the language and order, then use the Elementor widget **Учебные материалы**. To localize any static Elementor section, add **Контент по языку**, create one repeater row per supported language, and fill the content for each row; the widget follows `?vibelms_language=ru|kz` and the saved language cookie. Its HTML is sanitized and only HTTPS YouTube iframe hosts are retained. The widgets **Шапка VibeLMS** and **Подвал VibeLMS** provide reusable site chrome; built-in front-end labels follow the active language, and **Карточка курса** supports a separate title, description and button for every configured language. Static native Elementor text on the default homepage also follows the active Kazakh selector, including the Elementor editor preview, while the original widgets remain editable in Elementor. The same blocks are available as `[vibelms_header]`, `[vibelms_footer]` and `[vibelms_language_switcher]`. Configure the assessment rule, dashboard cards, certificate button and language groups under **VibeLMS → Настройки → Общие**: assign one access group to Русский and one to Казахский, and new participants will be enrolled automatically after choosing their language in the registration form. On the dashboard choose 7, 30, 90 days, the current month or a custom date range. Review **Журнал тестирования**, and check **VibeLMS → Статус → Логи** when `VIBELMS_DEBUG` is enabled. In **VibeLMS → Настройки → Общие → Режим интерфейса** choose **Упрощённый** for focused navigation or **Расширенный** to show the full legacy menu. The compact VibeLMS header includes materials for administrators and provides quick access to the main sections; the existing course builder remains unchanged. If a quiz request is repeated after completion, VibeLMS opens the saved result instead of returning a dead-end error; a genuine request failure includes **Начать заново**, including when the server renders the recovery message before the quiz UI loads.


The current package is `dist/vibelms-0.0.55.zip`.
